...
#    raw_input("Press Enter to continue analyzing tweets data...") # for DEMO purposes...
    ...
    print("Loading data...")
    ## load retweet, comments, favourtie data
    with open(os.path.join(data_dir, x_filename), encoding = "utf-8") as f:
        retweet_count = []
        favorite_count = []
        for i, line in enumerate(f):
            tweet_obj = json.loads(line.strip(), encoding='utf-8')
            retweet_count.append(tweet_obj['retweet_count'])
            favorite_count.append(tweet_obj['favorite_count'])
        x_social = np.column_stack((np.array(retweet_count), np.array(favorite_count)))

    ## load text only data     
    with open(os.path.join(data_dir, 'tweets_processed.txt'), 'r', encoding = "utf-8") as f:
        x_text = np.array(f.readlines())
            
    ## load text and emoji data     
    with open(os.path.join(data_dir, 'tweets_processed_emoji.txt'), 'r', encoding = "utf-8") as f:
        x_text_emoji = np.array(f.readlines())
    ...
    # Get accuracy for text only
    pred_text = np.empty([0, 10])
    avg_p = 0
    avg_r = 0
    avg_f1 = 0
    for train, test in kf.split(y):
        ...
        predict_scores_text = []
        for instance in x_text[test]:
            predict_scores_text.append(classifier.polarity_scores(instance)["compound"])
        predicts = map_to_label(predict_scores_text)
    ...
    # Get accuracy for text and emoji only
    pred_text_emoji = np.empty([0, 10])
    avg_p = 0
    avg_r = 0
    avg_f1 = 0
    for train, test in kf.split(y):

        predict_scores_text_emoji = []
        for instance in x_text_emoji[test]:
            predict_scores_text_emoji.append(classifier.polarity_scores(instance)["compound"])
        predicts = map_to_label(predict_scores_text_emoji)
        avg_p   += precision_score(y[test],predicts, average='macro')
        avg_r   += recall_score(y[test],predicts, average='macro')
        avg_f1   += f1_score(y[test],predicts, average='macro')

    print('Best Precision text + emoji is %f.' %(avg_p/10.0))
    print('Best Recall text + emoji is %f.' %(avg_r/10.0))
    print('Best F1 score is %f.' %(avg_f1/10.0))
  
############### Using social data to improve text + emoji data prediction #########################  
    W1 = np.arange(0,0.5,0.05)
    W2 = np.arange(0,0.5,0.05)
    results = []
    tweet_index = np.arange(0,4970,1)
    count = 0
...
#            for train, test in kf.split(y):
#                predict_scores_text_emoji = []
#                for count in tweet_index[test]:
#                    instance = x_text_emoji[count]
#                    predict_scores_text_emoji.append(classifier.polarity_scores(instance)["compound"]*(1+fave_w*favorite_count[count])*(1+retweet_w*retweet_count[count]))
#                predicts = map_to_label(predict_scores_text_emoji)
#                avg_p   += precision_score(y[test],predicts, average='macro')
#                avg_r   += recall_score(y[test],predicts, average='macro')
#                avg_f1   += f1_score(y[test],predicts, average='macro')
#            results.append([fave_w, retweet_w, avg_p/10, avg_r/10, avg_f1/10])
#            print([fave_w, retweet_w, avg_p/10, avg_r/10, avg_f1/10])
#    
#    best_results = results[np.argmax(results, axis=0)[4]]
#    print('Favourite weight is %f.' %(best_results[0]))
#    print('Retweet weight is %f.' %(best_results[1]))
#    print('Precision text + emoji + social is %f.' %(best_results[2]))
#    print('Recall text + emoji + social is %f.' %(best_results[3]))
#    print('F1 score text + emoji + social is %f.' %(best_results[4]))
    
    

############### late fusion model for social and text + emoji data #########################
      
     ## ML Model including both social emoji, text data 
#    # choose classifier for social
...
#        avg_f1 = 0
#        for train, test in kf.split(y):
#            model = modelx.fit(x_social[train], y[train]) 
#            predicts = model.predict(x_social[test])
#        avg_p	+= precision_score(y[test],predicts, average='macro')
#        avg_r	+= recall_score(y[test],predicts, average='macro')
#        avg_f1 += f1_score(y[test],predicts, average='macro')
#
#        print('Average Precision social is %f.' %(avg_p/10.0))
#        print('Average Recall social is %f.' %(avg_r/10.0))
#        print('Average F1 score is %f.' %(avg_f1/10.0))
#
#    # train score for social
#    pred_social = np.empty([0, 3])
#    for train, test in kf.split(y):
#        model = KNeighborsClassifier().fit(x_social[train], y[train]) 
#        predicts = model.predict_proba(x_social[test])
#        pred_social = np.concatenate((pred_social, predicts))
#
#    # train score for for text with emoji
#    pred_text_emoji = np.empty([0, 3])
#    for train, test in kf.split(y):
#        predict_scores_text_emoji = []
#        for instance in x_text_emoji[test]:
#            pred_score = [classifier.polarity_scores(instance)['neg'], classifier.polarity_scores(instance)['neu'], classifier.polarity_scores(instance)['pos']]
#            predict_scores_text_emoji.append(pred_score)
#
#        pred_text_emoji = np.concatenate((pred_text_emoji, predict_scores_text_emoji))
#
#
#    # Assign different weights to different classifer
#    grid = np.array([(T, S) for T in np.arange(0.8,1,0.005) for S in np.arange(0,0.2,0.005)])
#    
#    grid_new = []
...
#        combine_pred = grid_new[i][0]*pred_text_emoji + grid_new[i][1]*pred_social
...
#    print('Precision is %f.' %precisions[opt_id])
#    print('Recall is %f.' %recalls[opt_id])
#    print('F1 score is %f.' %f1_scores[opt_id])
#        
#
################### merge social, VADER outputs and put into ML model ###############################
...
#        count = 0
#        for instance in x_text_emoji[test]:
#            pred_score = [classifier.polarity_scores(instance)['neg'], classifier.polarity_scores(instance)['neu'], classifier.polarity_scores(instance)['pos'],classifier.polarity_scores(instance)['compound'], retweet_count[count], favorite_count[count]]
#            predict_scores_all.append(pred_score)
#            count += 1
#        pred_all = np.concatenate((pred_all, predict_scores_all))
#
#    # choose classifier
#    Model_test = [KNeighborsClassifier(), RandomForestClassifier()]
#
#    for modelx in Model_test:
#        avg_p = 0
#        avg_r = 0
#        avg_f1 = 0
#        for train, test in kf.split(y):
#            model = modelx.fit(pred_all[train], y[train]) 
#            predicts = model.predict(pred_all[test])
#        avg_p	+= precision_score(y[test],predicts, average='macro')
#        avg_r	+= recall_score(y[test],predicts, average='macro')
#        avg_f1 += f1_score(y[test],predicts, average='macro')
#
